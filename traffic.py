...
class Traffic(pygame.sprite.Sprite):
    def __init__(self, x):
        ...
        self.rect = self.image.get_rect(left = (self.x + 75.0), top = (self.y + 20.0), width = 90, height = 210)

    def drive(self):
        #Increases the object y value by the current speed, moving it toward the bottom of the screen
        randspeed = Decimal(self.randomspeed)
        roundspeed = round(randspeed, 2)
        print(roundspeed)
        self.y += roundspeed
        print(self.x, self.y)
        self.rect.move_ip(0.0, roundspeed)
    ...
    def reset(self):
        ...
        self.randomspeed = random.choice(self.speeds)
        self.image = random.choice(self.imagelist)
        self.rect = self.image.get_rect(left = (self.x + 71), top = (self.y + 17), width = 96, height = 214)
